...
- Bounded local template packs with no hooks, network access, extensions, or code execution.
- `samsarix inspect-template` for schema validation, file inventory, and provenance digest output.
- Read-only `samsarix plan` previews in human-readable and stable JSON formats.
...
- `samsarix init` accepts either a reviewed built-in or a local template pack and reports its exact
  identity after atomic generation.
- Structural checks remain compatible with schema-1 manifests generated by the 1.1 release line.
